  ...
  async function submit(event) {
    ...
    try {
      const result = await register({ 
        ...form, 
        studentId: form.userType === "STUDENT" ? form.studentId : undefined,
        departmentCode: (form.userType === "STAFF" || form.userType === "DRIVER") ? undefined : form.departmentCode
      });
  ...
  return (
    <AuthPageShell eyebrow="Create account" title="Join Safar" description="Register with your verified CUET identity. Transport Admin accounts are created only by the system administrator.">
      ...
      <form className="mt-6 grid gap-5 sm:grid-cols-2" onSubmit={submit}>
        ...
        {form.userType !== "STAFF" && form.userType !== "DRIVER" && <label className="block"><span className="text-sm font-semibold">Department</span><select className={fieldClass} name="departmentCode" value={form.departmentCode} onChange={update}>{departments.map((item) => <option key={item.value} value={item.value}>{item.value} — {item.label}</option>)}</select></label>}
        ...
        <div className="sm:col-span-2"><p className="mb-4 text-xs leading-5 text-safar-gray">Use at least eight characters with uppercase, lowercase, and a number.</p><Button className="w-full" type="submit" loading={loading}>Create account</Button></div>
      </form>
      <p className="mt-6 text-center text-sm text-safar-gray">Already registered? <Link className="font-semibold text-safar-teal" to="/login">Sign in</Link></p>
    </AuthPageShell>
  );
}
