    ...
    <DashboardLayout>
      <div className="space-y-6">
        <PageHeader
          ...
          description="Monitor every active bus with simulated GPS movement, current location, speed, ETA, and status."
          actions={<Badge tone={connectionStatus === "connected" ? "success" : "warning"}>{connectionStatus === "connected" ? "Live connection" : "Connecting"}</Badge>}
        />

        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm font-semibold text-jaatra-gray">{visibleBuses.length} buses visible</p>
          <label className="flex items-center gap-2 text-sm font-semibold text-jaatra-ink">Status<select className="focus-ring h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>{statuses.map((status) => <option key={status}>{status}</option>)}</select></label>
        </div>
        ...
        <section className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          {selected && (
            <aside className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
              <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase text-jaatra-teal">{selected.id}</p><h2 className="mt-1 text-xl font-bold text-jaatra-ink">{selected.name}</h2><p className="text-sm text-jaatra-gray">{selected.busNumber} | {selected.category}</p></div><LiveStatusBadge status={selected.status} /></div>
              <dl className="mt-6 divide-y divide-slate-100">
                ...
                ].map(([Icon, label, value]) => <div key={label} className="flex gap-3 py-4"><Icon className="h-5 w-5 shrink-0 text-jaatra-teal" /><div className="min-w-0"><dt className="text-xs font-bold uppercase text-jaatra-gray">{label}</dt><dd className="mt-1 break-words text-sm font-semibold text-jaatra-ink">{value}</dd></div></div>)}
              </dl>
            </aside>
          )}
        </section>
