...
export default function UserManagementPage() {
  ...
  const controls = (user) => <div className="flex flex-wrap items-center gap-2"><select aria-label={`Role for ${user.name}`} title={backendEnabled ? "Verified identity roles cannot be changed here" : undefined} disabled={backendEnabled} className="focus-ring h-9 rounded-lg border border-slate-200 px-2 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-60" value={user.role} onChange={(e) => changeUser(user.id, { role: e.target.value }, `${user.name}'s role changed to ${e.target.value}.`)}>{roles.map((item) => <option key={item}>{item}</option>)}</select><button className="focus-ring rounded-lg p-2 text-jaatra-teal hover:bg-jaatra-mint" aria-label={`View ${user.name}`} onClick={() => setToast({ type: "info", message: `${user.name}: ${user.universityId}, ${user.email}.` })}><Eye className="h-4 w-4" /></button><button className={`focus-ring rounded-lg p-2 ${user.status === "Active" ? "text-red-600 hover:bg-red-50" : "text-emerald-600 hover:bg-emerald-50"}`} aria-label={`${user.status === "Active" ? "Deactivate" : "Activate"} ${user.name}`} onClick={() => changeUser(user.id, { status: user.status === "Active" ? "Inactive" : "Active" }, `${user.name} is now ${user.status === "Active" ? "inactive" : "active"}.`)}>{user.status === "Active" ? <UserX className="h-4 w-4" /> : <UserCheck className="h-4 w-4" />}</button></div>;
  const columns = [
