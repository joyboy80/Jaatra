...
function VerticalBars({ values, labels }) {
  ...
  return <div className="flex h-56 items-end gap-2 sm:gap-3">{values.map((value, index) => <div key={`${labels[index]}-${index}`} className="flex h-full min-w-0 flex-1 flex-col justify-end"><p className="mb-2 text-center text-xs font-bold text-jaatra-ink">{value}</p><div className="w-full rounded-t-lg bg-jaatra-teal transition-all" style={{ height: `${Math.max(8, value / max * 78)}%` }} /><p className="mt-2 truncate text-center text-xs font-semibold text-jaatra-gray">{labels[index]}</p></div>)}</div>;
}

function HorizontalBars({ items }) {
  const max = Math.max(...items.map((item) => item.value), 1);
  return <div className="space-y-4">{items.map((item) => <div key={item.label}><div className="mb-1 flex justify-between gap-3 text-sm"><span className="truncate font-semibold text-jaatra-ink">{item.label}</span><span className="font-bold text-jaatra-teal">{item.value}%</span></div><div className="h-2.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-jaatra-teal" style={{ width: `${item.value / max * 100}%` }} /></div></div>)}</div>;
}

function ChartPanel({ title, description, children }) {
  return <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><h2 className="text-lg font-bold text-jaatra-ink">{title}</h2><p className="mt-1 text-sm text-jaatra-gray">{description}</p><div className="mt-6">{children}</div></section>;
}

export default function AnalyticsPage() {
  const [analytics, setAnalytics] = useState(null);
  useEffect(() => { getAnalytics().then(setAnalytics); }, []);
  if (!analytics) return <DashboardLayout><p className="text-sm font-semibold text-jaatra-gray">Loading analytics...</p></DashboardLayout>;
  ...
  return <DashboardLayout><div className="space-y-6"><PageHeader eyebrow="Transportation Intelligence" title="Analytics" description="Understand demand, occupancy, route performance, peak periods, cancellations, no-shows, and fleet use." />
    ...
    <section className="grid gap-4 xl:grid-cols-2">
      ...
      <ChartPanel title="Operational health" description="Key rates for service quality and fleet performance."><div className="grid gap-4 sm:grid-cols-3">{[["Utilization", analytics.utilization, "text-jaatra-teal"], ["Completed", 89, "text-emerald-600"], ["On-time", 84, "text-sky-600"]].map(([label, value, tone]) => <div key={label} className="rounded-xl bg-slate-50 p-4 text-center"><BarChart3 className={`mx-auto h-6 w-6 ${tone}`} /><p className="mt-3 text-2xl font-bold text-jaatra-ink">{value}%</p><p className="text-xs font-semibold text-jaatra-gray">{label}</p></div>)}</div></ChartPanel>
    </section>
  </div></DashboardLayout>;
}
