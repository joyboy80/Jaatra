          ...
          {routes.map((route) => (
            <article key={route.id} className="min-w-0 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-xs font-bold uppercase text-jaatra-teal">{route.id}</p><h2 className="mt-1 text-lg font-bold text-jaatra-ink">{route.name}</h2><p className="mt-1 text-sm text-jaatra-gray">{route.start} to {route.destination} | {route.estimatedMinutes} min</p></div>
                <div className="flex gap-1"><button className="focus-ring rounded-lg p-2 text-jaatra-teal hover:bg-jaatra-mint" aria-label={`Edit ${route.name}`} onClick={() => openForm(route)}><Pencil className="h-4 w-4" /></button><button className="focus-ring rounded-lg p-2 text-red-600 hover:bg-red-50" aria-label={`Delete ${route.name}`} onClick={() => setDeleteTarget(route)}><Trash2 className="h-4 w-4" /></button></div>
              </div>
              <div className="mt-5 overflow-x-auto pb-2">
                <div className="flex min-w-max items-start">
                  {route.stops.map((stop, index) => (
                    <div key={`${stop}-${index}`} className="flex items-start">
                      <div className="w-28 text-center"><div className={`mx-auto grid h-8 w-8 place-items-center rounded-full ${index === 0 || index === route.stops.length - 1 ? "bg-jaatra-teal text-white" : "bg-jaatra-sky text-jaatra-teal"}`}><MapPin className="h-4 w-4" /></div><p className="mt-2 text-xs font-semibold text-jaatra-ink">{stop}</p></div>
                      {index < route.stops.length - 1 && <div className="mt-4 h-0.5 w-8 bg-jaatra-teal/40" />}
                    </div>
                  ))}
                </div>
              </div>
              <div className="mt-4 flex items-center gap-2 text-sm text-jaatra-gray"><BusFront className="h-4 w-4 text-jaatra-teal" /> {route.assignedBusIds.length} buses assigned</div>
            </article>
          ))}
        </section>
      </div>
