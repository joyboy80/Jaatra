...
export default function FloatingAIChat() {
  ...
  return (
    <>
      {!open && <button type="button" className="ai-fab focus-ring fixed bottom-5 right-4 z-40 flex h-14 items-center gap-2 rounded-full px-4 text-sm font-bold text-white shadow-soft transition hover:-translate-y-0.5 sm:right-6" onClick={() => setOpen(true)} aria-label="Open Safar AI"><span className="relative"><Bot className="h-5 w-5" /><Sparkles className="absolute -right-2 -top-2 h-3.5 w-3.5 text-cyan-200" /></span><span>Safar AI</span></button>}
