  ...
  return (
    <article className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-soft">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-base font-bold text-safar-ink">{bus.name}</h3>
          <p className="text-sm text-safar-gray">{bus.number} / {bus.type}</p>
        </div>
        <Badge tone={toneForStatus(bus.status)}>{bus.status}</Badge>
      </div>
      <div className="mt-4 space-y-2 text-sm text-safar-gray">
